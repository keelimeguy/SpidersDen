...
public abstract class Projectile extends Entity {
	...
	public int getEdgePinX(int c, int dx, int xOff, boolean mirror) {
		if (c < 2)
			...
			return ((int) (x + dx) + (((c - 2) % 2) == 0 ? 1 : 0) * (sprite.SIZE_X / 2 + xOff) - sprite.SIZE_X / 4 - (mirror ? ((c - 2) % 2) * xOff : 0)) >> 4;
		else
			return ((int) (x + dx) + (((c - 2) % 2) == 0 ? 1 : 0) * sprite.SIZE_X / 2 + ((c - 2) % 2) * xOff - sprite.SIZE_X / 4 - (mirror ? ((c - 1) % 2) * xOff : 0)) >> 4;
	}
	...
	public int getEdgePinY(int c, int dy, int yOff, boolean mirror) {
		if (c >= 2)
			...
			return ((int) (y + dy) + ((((c % 2) + 1) % 2) == 0 ? 1 : 0) * sprite.SIZE_Y / 2 - (((c % 2) + 1) % 2) * yOff - sprite.SIZE_Y / 4 + (mirror ? (c % 2) * yOff : 0)) >> 4;
		else
			return ((int) (y + dy) + ((((c % 2) + 1) % 2) == 0 ? 1 : 0) * sprite.SIZE_Y / 2 - sprite.SIZE_Y / 4 - (c % 2) * yOff + (mirror ? ((c + 1) % 2) * yOff : 0)) >> 4;
	}
